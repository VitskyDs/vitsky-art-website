...
export async function PrintsPageContent({ locale }: Props) {
  const payload = await getPayload({ config: configPromise })

  // Find the "prints" category
  const categoriesResult = await payload.find({
    collection: 'categories',
    locale,
    where: { slug: { equals: 'prints' } },
    limit: 1,
  })
  ...
  return (
    <div className="container my-16">
      ...
      {products.docs.length > 0 ? (
        ...
        <p className="text-neutral-500">{t('prints.empty', locale)}</p>
      )}
    </div>
  )
}
